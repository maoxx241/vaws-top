...
唯一版本号来自 `pyproject.toml` 的 `0.1.4`。MCP `serverInfo.version` 读取 `importlib.metadata.version("vaws-top")`；`package.json` 的 version 与之相同。
...
```bash
gh release download v0.1.4 -R vllm-ascend-workspace/vaws-top -p '*.whl'
uvx --from ./vaws_top-0.1.4-py3-none-any.whl vaws-top serve
```

或在已具备仓库读权限的环境里直接指向资产 URL：

```bash
uvx --from "https://github.com/vllm-ascend-workspace/vaws-top/releases/download/v0.1.4/vaws_top-0.1.4-py3-none-any.whl" vaws-top serve
```
...
```json
{
  "mcpServers": {
    "vaws-top": {
      ...
      "args": [
        "--from",
        "https://github.com/vllm-ascend-workspace/vaws-top/releases/download/v0.1.4/vaws_top-0.1.4-py3-none-any.whl",
        "vaws-top",
        "mcp"
      ],
      "env": {
